      ...
      <div className="grid md:grid-cols-2 gap-4">
        {goals.map((goal) => (
          <motion.button
            ...
            className={`p-5 rounded-xl border text-left transition-all duration-200 flex items-center gap-3 hover:bg-secondary/10 ${
              formData.goals.includes(goal)
                ? 'border-secondary bg-secondary/20 text-white'
                : 'border-white/10 bg-white/5 text-white/80 hover:border-secondary/50'
            }`}
          >
            <div className={`w-5 h-5 rounded border flex items-center justify-center ${formData.goals.includes(goal) ? 'border-secondary bg-secondary' : 'border-white/20'}`}>
