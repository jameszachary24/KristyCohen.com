...
const Step2_Revenue = ({ formData, updateData, nextStep }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-white/60 mb-8">This helps us recommend the right strategy for your stage.</p>
      <div className="space-y-4">
        {options.map((option) => (
          <motion.button
            ...
            className={`w-full p-6 rounded-2xl border text-left transition-all duration-200 flex items-center justify-between group hover:bg-secondary/10 ${
          ...
          >
            <div>
              <div className="font-bold text-lg group-hover:text-secondary transition-colors">{option.label}</div>
              <div className="text-sm text-white/60">{option.desc}</div>
            </div>
            <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${formData.revenue === option.id ? 'border-secondary bg-secondary' : 'border-white/20'}`}>
